...
logger = create_logger('child','logs/Child.log', logging.DEBUG, logging.WARNING)
...
class Child_Capabilities():
	def add_feature(self,new_feature:pd.DataFrame):
		...
		common_elements = list(set(new_feature_list).intersection(set(self.data.columns)))
		...
		if len(new_feature)==len(self.data):
			...
			logger.warning(f"Length of new_features: {len(new_features)} and current data: {len(self.data)} is not same")
